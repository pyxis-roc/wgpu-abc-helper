...
(define-const wrap_low BV16 (wrap_count (extended_mul x lower_bound false)))
(define-const wrap_high BV16 (wrap_count (extended_mul x upper_bound false)))
...
(assert (bvule (bvsub (extended_mul x upper_bound false) (extended_mul x lower_bound false)) #x00ff))
; (assert (bvult wrapped_low wrapped_high))
(check-sat)
(pop)
...
(assert (= (bvadd #x0001 wrap_low) wrap_high))
(assert (bvule (bvsub (extended_mul x upper_bound false) (extended_mul x lower_bound false)) #x00ff))
(assert (bvugt wrapped_low wrapped_high))
(check-sat)
(pop)

(push)
(assert (= wrap_low wrap_high))
(assert (bvugt wrap_low (_ bv0 16)))
